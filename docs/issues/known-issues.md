...
This document tracks ongoing issues with the MediaTek MT7925 WiFi driver that are **partially mitigated** by the patches in this repository but may have root causes in the firmware itself.

## Issue #1: MCU Timeout During MLO Roaming

| | |
|---|---|
| **Status** | :material-alert:{ .yellow } Partially Mitigated |
| **Severity** | Medium |
...
During MLO roaming, the driver fails to provide BSS information for the target link.
...
|-------|-----------|---------------------|
| Key removal -EINVAL | :material-check: Patch 0012 | No (driver bug) |
| BSS fetch failure | :material-check: NULL checks help | Yes |
| MCU timeout recovery | :material-check: Patches prevent crash | Root cause in FW |
| Firmware hang | :material-check: Auto-recovery works | Shouldn't happen |

### What Our Patches Do

**Without patches**, this sequence would cause:
...
| | |
|---|---|
| **Status** | :material-alert-circle:{ .red } Unmitigated (Firmware/Hardware) |
| **Severity** | Medium |
...
3. Check if our patches are applied: `dkms status | grep mt76`
4. Open an issue on [GitHub](https://github.com/zbowling/mt7925/issues)
